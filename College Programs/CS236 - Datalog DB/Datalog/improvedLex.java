...
//       Token getNextToken()   -- Advances to the next token and returns it
//
//   There aren't any other methods you should use in your Parser
//--------------------------------------------------------------------------
...
class Token{
  ...
  //used an enumerated type to define type but Java does not have one so we
  //do it with a bunch of final static ints.
  ...
  String[] typeStrings = { //Usefule for debugging
	     "COLON",   "COLON_DASH",   "COMMA",   "LEFT_PAREN",   "PERIOD",   "Q_MARK",   "RIGHT_PAREN",   "EQ", "NE",   "GT",
	     "GE",   "LT",   "LE",   "STRING",   "SCHEMES",   "FACTS",   "RULES",   "QUERIES",   "ID",   "EOF",   "UNDEFINED"  };
...
class LexicalAnalyzer{
  ...
  //We implement the lexical analyzer with a one character buffer.  That is,
  //the next character to be read is in a local character, "current_character"
  //and the rest of the characters are in the input file.  Thus, we can check
  //to see if we are at the end of a file by checking to see if the
  //current_character is -1.  We also use this feature to be able to
  //differentiate between one and two character symbols such as ":-" and ":".
  //The one character buffer basically provides a one character look ahead into
  //the input file.  Thus, after seeing a ":" we can look one character ahead
  //to see if we should return a ":-" or just a ":".  Similar decisions are
  //made for "<=" vs. "<" and ">=" vs. ">".
  ...
  public Token currentToken(){return curr_token;};
	...
	public Token getNextToken(){//Parallels how advancing tokens is talked about in class
		nextToken();
		return currentToken();
	}
  ...
  //the current character is an int rather than a char so that eof (-1) can
  //be stored and tested.
  ...
  //Changes the token type appropriately if the value of the token type
  //is a keyword.  Otherwise it leaves the type the way it was.
