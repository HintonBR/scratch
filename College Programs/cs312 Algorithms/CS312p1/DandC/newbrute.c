...
int combine(Pair *pr, int s, int e) {
	int i = s, j = e, c, d;
	double temp ;
	Pair *p;
	...
	if (j > i) {
		c = 0;
		for (i; i <= j; i++) {
			tempPoints[c] = points[i];
			c++;
		}
		qsort(tempPoints, j - i +1, sizeof(Point), compare_y);
		for (i = 0; i < j; i++) {
			for (d=i+1; d <= ((j - i) > 7 ? i+7 : j); d++) {
				temp =  distance(tempPoints[i],tempPoints[d]);
				if (temp < pr->dist)
				{
				  pr->dist = temp;
				  pr->p1.x = points[i].x;
