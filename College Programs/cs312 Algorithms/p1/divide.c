...
int main(int argc, char *argv[])
{
 ...
 unsigned long mine = 0;  time( &start );
  if (argc < 2 )
  {
...
Pair *DandC(unsigned long s, unsigned long e, Point currY[], unsigned long sizey) {
	Pair *p1, *p2;
	Point* temp1 = (Point*)malloc(sizeof(Point) * (sizey + 1)), *temp2  = (Point*)malloc(sizeof(Point) * (sizey + 1));
	...
	arrayBound = s + (e-s)/2;	
        ...
        #endif
        #ifdef debug2 	
		 for (mine = 0; mine < sizey; mine++) {
			printf("The y1array (%lu, %lu)\n", currY[mine].x,currY[mine].y);
		}
        #endif
 	if ((e - s + 1) <= 3) {
		...
		return p1;
			...
			if (currY[count].x < points[arrayBound].x) {
                ...
                #ifdef d
                 printf("Array size of first array is %lu\n", itemp1);
                 printf("Array size of second array is %lu\n", itemp2);
                #endif
                #ifdef debug1
                 for (mine = 0; mine < itemp1; mine++) {
	printf("The y1array (%lu, %lu)\n", temp1[mine].x,temp1[mine].y);
}
 for (mine = 0; mine < itemp2; mine++) {
	printf("The y2array (%lu, %lu)\n", temp2[mine].x,temp2[mine].y);
}
                #endif
...
void combine(Pair *pr, unsigned long arrayBound, Point yarray[],unsigned long yarraysize) {
         Point* ytemp=(Point*)malloc(sizeof(Point) * (yarraysize + 1));
         Pair *p;
...
{
//  FILE *fp;
  unsigned long numpoints,numlines, sum, true_sum, i,x, filesize;
  int fp ;
  char * Buffer;
  char num_ptr[20];
  char ch;
