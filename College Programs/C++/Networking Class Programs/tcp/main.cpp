...
// The NAT table is included so you dont have to rewrite your IP layer to 
//  compile
...
// Initialize the protocols
  ...
  /* Send different sizes */
  memcpy(&tcpaddr[0],dest_tcpaddr,TCP_ADDRESS_LEN);
  memcpy(&tcpaddr[TCP_ADDRESS_LEN],dest_ipaddr,IP_ADDRESS_LEN);
  print_chars("main sending small message ", buff, len);
  print_addr("to", (unsigned char *)tcpaddr, TCP_ADDRESS_LEN + IP_ADDRESS_LEN);
  
  tcpp->push (buff, len, tcpaddr, NULL);
  
  for(i = 0; i < sizeof(buff); i++)
    buff[i] = i;
  
  print_chars("main sending 500 byte message ", buff, 500);
  print_chars("to", (unsigned char *)tcpaddr, TCP_ADDRESS_LEN + IP_ADDRESS_LEN);
  tcpp->push (buff, 500, tcpaddr, NULL);
