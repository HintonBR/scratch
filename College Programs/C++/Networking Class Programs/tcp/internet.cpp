 ...
 char *addr, int port, int client)
{
  ...
  memset(frag4.packet, 0, 1000);
  memset(frag5.packet, 0, 1000);
  memset(frag6.packet, 0, 1000);
}
  ...
  if (memcmp(destIP, myipaddr,4) == 0) {
	  if (csum == calculateCheckSum((u_short*)buf,10)) {
		...
		if (offset == 0) {
			...
			else if (frag4.inuse == false) {
				curFrag = &frag4;
			}
			else if (frag5.inuse == false) {
				curFrag = &frag5;
			}
			else if (frag6.inuse == false) {
				curFrag = &frag6;
			}
			else {
				curFrag = &frag3;
