...
// Grab the type out of the top of the length parameter and send it
  ...
  if(blabby & 0x2) {
    ...
    print_addr("Push Ethernet DST Address", addr, ETH_ADDRESS_LEN);
...
// Byte swap, we need to change this to a htons
  ...
  if(prot_lower)
    prot_lower->push ((unsigned char *)ethmessage, msglength+14, 0, NULL);
...
int ethernet::pop (unsigned char *buf, int len, message *mb)
{
  ...
  if(prot_higher)
    prot_higher->pop ((unsigned char *)&buf[14], combined, mb);
  return(1);
}
