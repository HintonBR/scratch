...
#define OTHER2_IP {192,168,102,246}
...
int main (int argc, char* argv[])
{
  ...
  if(strcmp(argv[7],"-s") == 0) { // This is the server
    arpsend->push (buff, 4, my_ip, NULL);
  } else {
    int ch;
    ...
    for( i = 0; (i < MAXLEN) &&  ((ch = getchar()) != '\n'); i++ )
      inbuff[i] = (char)ch;
    arpsend->push (inbuff, i, dest_ip, NULL);
    // Now send the test pattern
    arpsend->push (buff, 4, dest_ip, NULL);
    arpsend->push (buff, 4, other1_ipaddr, NULL);
    arpsend->push (buff, 4, other2_ipaddr, NULL);
