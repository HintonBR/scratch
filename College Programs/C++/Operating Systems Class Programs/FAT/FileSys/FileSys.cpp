...
int smain() {
	...
	if (fs->isValid() == 0) {
		...
		//fs->printbs();
		//cout << fs->sizeOfRoot << endl;
		//fs->printRootDirectory();
		
		cout << "The number of free fat entries is:" << fs->getnumfree() << endl;
		fs->fdListDir(&output);
		cout << output;
		cout << endl;
		cout << "The number of free fat entries is:" << fs->getnumfree() << endl;
		//fs->fdEraseFile("test4");
		//fs->fdDelDir("File1.dir");
		//fs->fdChangeDir("dir1");
		//fs->fdMakeDir("mydir");
		fs->fdEraseFile("file3.txt");
		fs->fdDelDir("mydir");
		fs->fdMakeDir("mystuff");
		fs->fdDelMem(&output);
		fs->fdListDir(&output);
		cout << output;
		cout << endl;
		fs->fdChangeDir("mystuff");
		while (1) {
		cluster = fs->fdOpenFile("file21.txt", 1);
		//fs->fdCloseFile(cluster);
		cout << cluster << endl;
		//cout << fs->fdCloseFile(cluster) << endl;
		//fs->fdSeek(cluster, 334000);
		memset((void*) buf, 0, 200000);
		//for (count = 0; count <10; count++) {
		//strcat(buf, "Hellotttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt\n");
		//}
		strcpy(buf, "lkasjdf;lksajdf;lksahf;iashnfa;ksdfjahsldfkjashdfkjsahdflkjsadhfaslkjfdhasldkjfhasdlkjfhasdlkjfhsadlkjfhsadlkjfhasdlkjfhsadlkjfashdlkfjasdhflkjdshflkjdshflsakjdhfaslkjdjaksdhfaslkjdfh");
		fs->fdSeek(cluster, 150000);
		fs->fdSeek(cluster, 0);
		fs->fdWriteFile(cluster, buf, 100);
		fs->fdWriteFile(cluster, buf, 100);
		fs->fdWriteFile(cluster, buf, 100);
		fs->fdSeek(cluster, 0);
		cout << fs->fdReadFile(cluster, buf, 1000) << endl;
		cout << buf << endl;
		//fs->fdSeek(cluster, 600000);
		fs->fdCloseFile(cluster);
		}
		fs->fdDelMem(&output);
		fs->fdListDir(&output);
		...
		//fs->fdChangeDir("..");
		//fs->fdChangeDir("dir2");
		//fs->fdCopyFile("file5.txt","file51.txt");
		//fs->fdCopyFile("file5.txt","file52.txt");
		//fs->fdChangeDir("..");
		//fs->fdDelDir("dir1");
		//fs->fdChangeDir("..");
		//fs->fdDelMem(&output);
		...
		//cout << output;
		//cout << endl;
		//cluster = fs->GetCluster(cluster);
		//fs->fdEraseFile("test.txt");
		//	fs->fdDelMem(&output);
		//	fs->fdListDir(&output);
		//cout << output;
		//cout << endl;
		//fs->fdChangeDir("file1.dir");
		
		/*if (fs->fdChangeDir("test")) {
			fs->fdDelMem(&output);
			fs->fdListDir(&output);
			cout << output;
			cout << endl;
			if (fs->fdChangeDir("..")) {
				...
				fs->fdListDir(&output);
				cout << output;
				if (fs->fdEraseFile("test")) cout << "shouldn't have deleted" << endl;
				else cout << "it didn't delete" << endl;
				if (fs->fdDelDir("test")) {
					fs->fdDelMem(&output);
					fs->fdListDir(&output);
					cout << output;
		...
		//	fs->fdEraseFile("file1.");
	//		fs->fdEraseFile("file2");
	//		fs->fdDelMem(&output);
	//		fs->fdListDir(&output);
	//		cout << output << endl;
	//	fs->fdCopyFile("file2.txt", "file8");
	//	fs->fdCopyFile("file2.txt", "file8.");
	//	fs->fdCopyFile("file3", "file8.txt");
	//	fs->fdDelMem(&output);
	//		fs->fdListDir(&output);
	//		cout << output << endl;
	//	fs->findClusterNum("test4.txt");
	//	fs->printClusterList(fs->curDirectory);
	//	fs->fdCopyFile("test4.txt", "test5.txt");
	//	fs->fdDelMem(&output);
	//	fs->fdListDir(&output);
	//	cout << output << endl;
		cout << "The number of free fat entries is:" << fs->getnumfree() << endl;
