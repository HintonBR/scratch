...
(define apply-proc
  (lambda (proc args)
    (begin
      (write (prim-proc->prim-op proc))
    (variant-case proc
      (prim-proc (prim-op arity) (if (equal? (length args) arity) (begin (write "this is a test") (apply-prim-op prim-op args)) (error "Invalid number of arguments given to procedure:" proc)))
      (else (error "Invalid procedure:" proc))))))
