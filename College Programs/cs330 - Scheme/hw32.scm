...
(define eval-exp
  (lambda (exp env class inst)
    (variant-case exp
      ...
      (meth-app (name rands) (eval-exp (make-app name rands) env class inst))
      (method (formals body)
      ...
      (app (rator rands)
	(let ((args (eval-rands rands env class inst)))
	  (if (instance? (car args)) (meth-call rator (instance->class (car args)) args)
	  (let ((proc (eval-exp rator env class inst))) (apply-proc proc args class inst)))))
