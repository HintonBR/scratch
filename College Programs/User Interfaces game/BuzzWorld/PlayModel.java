...
public class PlayModel extends Object implements java.io.Serializable {
    ...
    private void randomizeLocation(CharacterInstance c, SpaceModel p) {
        ...
        rand = Math.random();
        c.setLocation(new Point((int)(rand*rw), (int) (rand*rh)));
        Object o = getObjectAt(c, p);
        while (numtimes < 10 && (o instanceof CharacterInstance || c.getX() <0 || c.getY() < 0 || c.getX() + c.getSize().getWidth() > p.getImageSize().getWidth() || c.getY() + c.getSize().getHeight() > p.getImageSize().getHeight())) { 
            rand = Math.random();
            c.setLocation(new Point((int)(rand*rw), (int) (rand*rh)));
            o = getObjectAt(c, p);
            numtimes++;
        ...
        if (numtimes == 10) {
            ...
            c.setLocation((int) (mid.getX() - c.getSize().getWidth()/2), (int) (mid.getY() - c.getSize().getHeight()/2));
        }
    }
    public Object getObjectAt(CharacterInstance c, SpaceModel sp) {
        CharacterInstance ci;
        Rectangle source = c.getRectangle();
        Portal p;
        ...
        while (it.hasNext()) {
            p = (Portal) it.next();
            destination = new Rectangle((int) p.getX(), (int) p.getY(), (int) p.getWidth(), (int)p.getHeight());
    ...
    public Object getObjectAt(Rectangle source, SpaceModel sp) {
        ...
        while (it.hasNext()) {
            ci = (CharacterInstance) it.next();
            destination = new Rectangle((int) ci.getX(), (int) ci.getY(), (int) ci.getSize().getWidth(), (int)ci.getSize().getHeight());
    ...
    private void playModelPropertyChange(java.beans.PropertyChangeEvent evt) {
        ...
        if (evt.getPropertyName().equals(CharacterInstance.CI_CHARACTERMODEL) || evt.getPropertyName().equals(CharacterInstance.CI_CURRENTSTRENGTH)) { 
            ...
            //If object is a portal - check to see if we can teleport
                ...
                //teleport to.
                o = getObjectAt(new Rectangle((int) (mid.getX() - ci.getSize().getWidth()/2),(int) (mid.getY() - ci.getSize().getHeight()/2),(int)ci.getSize().getWidth(),(int)ci.getSize().getHeight()), p.getSpaceModel());
                if (o instanceof CharacterInstance) {
                     c = (CharacterInstance) o;
                    if (c.getTeam() != ci.getTeam()) {
                        ci.attack(c);
                    ...
                    mid.setLocation((int) (mid.getX() - ci.getSize().getWidth()/2),(int) (mid.getY() - ci.getSize().getHeight()/2));
                    ci.setLocation(mid);
                }
            }
